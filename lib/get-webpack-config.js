...
module.exports = function (options) {
    ...
    const userWebpackConfig = fileExists(userWebpackConfigPath) ?
    ...
    // === 插件配置
    /* istanbul ignore if  */
    if (!isArray(webpackConfig.plugins)) {
    ...
    if (!userWebpackConfig.override) { // 混合配置
        ...
        // 在项目中可缩减引用路径
        /* istanbul ignore if  */
        if (!isObject(webpackConfig.resolve)) {
            webpackConfig.resolve = {};
        }
        /* istanbul ignore if  */
        if (!isObject(webpackConfig.resolve.alias)) {
            webpackConfig.resolve.alias = {};
        }

        if (isUndefined(webpackConfig.resolve.alias['root'])) {
