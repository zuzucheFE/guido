...
module.exports = function (options) {
    ...
    options.devServerConfig = pkg.build.devServer || null;
    ...
    if (options.devServer) {
        ...
        if (options.devServerConfig && options.devServerConfig.router) {
            webpackConfig.devServer.historyApiFallback = options.devServerConfig.router;
        }
        Object.getOwnPropertyNames(webpackConfig.entry).map(function (name) {
            webpackConfig.entry[name] = []
                .concat(webpackConfig.entry[name])
                .concat('webpack-dev-server/client?http://' + webpackConfig.devServer.host + ':' + webpackConfig.devServer.port)
        });
    }
