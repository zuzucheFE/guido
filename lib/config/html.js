...
function toRelativeOutputDirPath (outputType, result) {
    ...
    let rootPath = path.normalize(path.join(outputTemplateDir, result));

    let outputDir = path.join(outputAssetsDir.output, outputAssetsDir[outputType]) + '/';
    rootPath = rootPath.replace(outputDir, '');

    return rootPath;
}
function toRelativeCssDirPath (result) {
    return toRelativeOutputDirPath.call(this, 'css', result);
}
function toRelativeJsDirPath (result) {
    return toRelativeOutputDirPath.call(this, 'js', result);
    // let self = this;
    //
    // if (ReInlinePlaceholder.test(result)) {
    //     result = result.replace(ReInlinePlaceholder, '');
...
module.exports = function (options, webpackConfig) {
    let htmlDirCwd = path.join(webpackConfig.context, 'src', webpackConfig.output.templateDir);
        ...
        htmlFiles.forEach(function (file) {
            ...
            if (/[\.\-]?module$/.test(info.name) === false) {
                ...
                result.push(new HtmlWebpackPlugin({
                    filename: path.join(webpackConfig.output.path, webpackConfig.output.templateDir, info.name + '.html'),
