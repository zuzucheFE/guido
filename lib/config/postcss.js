...
module.exports = function (options, webpackConfig) {
    ...
    function spritesOnSaveSpritesheet(opts, spritesheet) {
        ...
        return path.join(webpackConfig.context, options.tmpCacheDir, file.join(''));
    }

    return function () {
        return [
            autoprefixer({
                browsers: webpackConfig.browserslist || ['last 2 versions', 'Firefox ESR', '> 1%', 'ie >= 8', 'iOS >= 8', 'Android >= 4']
            }),
            postcssSprites({
                verbose: false,
            ...
            postcssIconFont({
                outputPath: path.join(webpackConfig.context, options.tmpCacheDir),
